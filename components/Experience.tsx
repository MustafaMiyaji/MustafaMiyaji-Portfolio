...
const ExperienceCard: React.FC<{ exp: typeof experiences[0]; index: number }> = ({ exp, index }) => {
    ...
    return (
        <div className={`relative flex items-center justify-between md:justify-normal w-full mb-8 ${isEven ? 'md:flex-row-reverse' : ''}`}>
            ...
            <div className="absolute left-4 md:left-1/2 w-4 h-4 -translate-x-1/2 bg-slate-900 border-2 border-cyan-500 rounded-full z-20 shadow-[0_0_10px_rgba(6,182,212,0.8)]">
            ...
            >
                ...
                <div className="relative p-6 bg-white/50 dark:bg-black/40 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden hover:border-cyan-500/30 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {exp.tech.map((t, i) => (
                            <span key={i} className="px-2 py-0.5 text-[10px] font-mono border border-slate-200 dark:border-white/10 rounded text-slate-500 dark:text-slate-400 hover:text-cyan-500 hover:border-cyan-500/30 transition-colors cursor-default">
                                {t}
                            </span>
                        ))}
                    </div>
